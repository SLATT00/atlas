...
import (
	...
	"github.com/atlas-platform/atlas/backend/internal/ledger/handler"
...
func main() {
	...
	ledger := r.Group("/api/v1/ledger")
	ledger.Use(middleware.Auth(cfg.JWT.Secret))
	{
		ledger.POST("/entries", h.CreateEntry)
		ledger.GET("/entries/:id", h.GetEntry)
		ledger.GET("/accounts/:account_id/balance", h.GetBalance)
		ledger.GET("/accounts/:account_id/entries", h.ListEntries)
	}
