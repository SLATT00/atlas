...
import (
	...
	"github.com/atlas-platform/atlas/backend/internal/exchange/handler"
...
func main() {
	cfg := config.Load("exchange-service")
	cfg.Port = 8009
	cfg.Postgres.DBName = "exchange_db"
	...
	exchange := r.Group("/api/v1/exchange")
	exchange.Use(middleware.Auth(cfg.JWT.Secret))
	{
		exchange.POST("/quote", h.GetQuote)
		exchange.POST("/execute", h.Execute)
		exchange.GET("/rates", h.GetRates)
		exchange.GET("/history", h.GetHistory)
	}
