...
import (
	...
	"github.com/atlas-platform/atlas/backend/internal/wallet/handler"
...
func main() {
	...
	wallets.Use(middleware.Auth(cfg.JWT.Secret))
	{
		wallets.GET("", h.ListWallets)
		wallets.GET("/:id", h.GetWallet)
		wallets.POST("/send", h.Send)
		wallets.POST("/receive", h.GenerateAddress)
		wallets.GET("/history", h.GetHistory)
	}
